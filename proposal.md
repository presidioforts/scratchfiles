...
**Summary**  
In 2025, the EPL-X resource planning strategy focuses on managing a projected ~10,000 annual tickets while supporting ~10,000 migrated components, stabilizing the platform, and enabling efficient developer self-service. This approach anticipates the evolving complexity of the EPL-X (GITSAS, GITACT) product and infrastructure environment. Three teams will continue to handle EPL-X Platform Services—Infrastructure and Operations:
...
**Primary Responsibilities:**  
- Manage and resolve standard workflow tickets across 9 buildpacks (e.g., Gradle-Java, .NET, npm, Python), including: Checkout, Build, Test, Publish, Code Sign, Secrets Management, Sonar Scanning, and Security Scan.  
...
- Deploy self-service automation tools for workflow input variables, enabling developers to configure and troubleshoot their EPL-X workflow pipelines independently.  
...
- Each engineer can effectively handle ~1,096 tickets/year after accounting for PTO/training.  
...
- With process improvements, each engineer can handle approximately 60 tickets/week (≈ 240 tickets/month).  
- After accounting for PTO/training (~17%), a safe estimate is ~1,096 tickets/engineer/year.

**Total Resource Requirement:**  
- To handle 10,000 tickets annually: ~10–11 engineers.

*(Note: The exact capacity and annualization should be validated. The current calculations assume monthly capacities scale linearly.)*
